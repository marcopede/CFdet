...
//end bow
    ...
    //printf("BOW%f Norm%d ",bowscr,toton);
    //if (voc[0]==10.0)
    //    return bowscr;
...
void hog2bow(ftype *img,int imgx,int imgy,ftype *mask,int masky,int maskx,int dimz,int posy,int posx,int sizevoc,int *code)
{
    //bow extraction
    int dimz2=13;
    int x,y,z,cy,cx,posi,c,maxpos,aux,pos;
    ftype maxval,val;
    //printf("Mask%f ",mask[0]);
    for (x=0;x<maskx-sizevoc+1;x++)
    {
        for (y=0;y<masky-sizevoc+1;y++)
        {
            ...
            for (c=0;c<sizevoc*sizevoc;c++)
            {
                ...
                if (maxval==0.0)//outside image
                    maxpos=NUMOR;
                aux=9+norm[c];
                if (buffer[aux+c*dimz2]<0.1)//low contrast
                    maxpos=NUMOR;
                /*if (voc[0]==10.0)
                    printf("%d %3f ",maxpos,maxval);*/
                pos+=maxpos*pow6[c];//compute the hash code
...
inline ftype refineighfullbow(ftype *img,int imgy,int imgx,ftype *mask,int masky,int maskx,int dimz,ftype dy,ftype dx,int posy,int posx,int rady,int radx,ftype *scr,int *rdy,int *rdx,ftype *prec,int pady,int padx,int occl,int sizevoc,int numvoc,ftype *voc,ftype *mhist)
{
    ...
    for (iy=-rady;iy<=rady;iy++)
    {
        for (ix=-radx;ix<=radx;ix++)
        {
            val=corr3dpadbow(img,imgy,imgx,mask,masky,maskx,dimz,posy+iy,posx+ix,prec,pady,padx,occl,sizevoc,numvoc,bcode+(ix+radx)+(iy+rady)*codex,codex,mhist)+k*k*dy*(iy*iy)+k*k*dx*(ix*ix);
            scr[(iy+rady)*(2*radx+1)+(ix+radx)]+=-val;
            if (val>maxval)
            {
                maxval=val;
...
void scanDefbow(ftype *ww1,ftype *ww2,ftype *ww3,ftype *ww4,int fy,int fx,int dimz,ftype *df1,ftype *df2,ftype *df3,ftype *df4,ftype *img,int imgy,int imgx,int *posy,int *posx,int *parts,ftype *res,int rad,int len,int usemrf,ftype *uscr,int useusrc,ftype *prec,int pady,int padx,int occl,int sizevoc,int numvoc,ftype *voc,ftype *mhist)
{
    //printf("dimz:%d occl:%d",dimz,occl);
    int sizedf=4;
    ftype maxscr1,maxscr2,maxscr3,maxscr4;
    ...
    for (i=0;i<len;i++)//for each location
    {
        ...
        //compute the score of the 4 parts
        maxscr1 = refineighfullbow(img,imgy,imgx,ww1,fy,fx,dimz,df1[0],df1[1],posy[i],posx[i],rad,rad,scr,dy,dx,prec1,pady,padx,0,sizevoc,numvoc,voc,mhist);
        maxscr2 = refineighfullbow(img,imgy,imgx,ww2,fy,fx,dimz,df2[0],df2[1],posy[i],posx[i]+fx,rad,rad,scr+(2*rad+1)*(2*rad+1),dy+1,dx+1,prec2,pady,padx,0,sizevoc,numvoc,voc,mhist+numvoc);
        maxscr3 = refineighfullbow(img,imgy,imgx,ww3,fy,fx,dimz,df3[0],df3[1],posy[i]+fy,posx[i],rad,rad,scr+3*(2*rad+1)*(2*rad+1),dy+2,dx+2,prec3,pady,padx,0,sizevoc,numvoc,voc,mhist+2*numvoc);
        maxscr4 = refineighfullbow(img,imgy,imgx,ww4,fy,fx,dimz,df4[0],df4[1],posy[i]+fy,posx[i]+fx,rad,rad,scr+2*(2*rad+1)*(2*rad+1),dy+3,dx+3,prec4,pady,padx,0,sizevoc,numvoc,voc,mhist+3*numvoc);
        if (usemrf)//use lateral constraints
        {
            for (c=0;c<4*(2*rad+1)*(2*rad+1);c++)
                scr[c]=-scr[c];//invert the score to keep compatibility
