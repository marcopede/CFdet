...
#    raw_input()
    ...
    for bost in range(20):
        ...
        for ex in final_trPosImages:
            if ex["name"].split("/")[-1] in dtrpos:
                ...
                if usex["bbox"]==[]:
                    ...
                    usex["bbox"]=[usex["bbox"][0]]#for semplicity use only 1st bbox per image
                    trPosImages=[usex]
                    end=False
                    break
            
        if end:
            print "Bosting Training Finished!!!"
            sfsd
            raw_input()

        cfg.numcl+=1
        lfy,lfx=buildcl(trPosImages,cfg)
        numcl=cfg.numcl
        #raw_input()

        cfg.fy.append(lfy[0])#[7,10]#lfy
        cfg.fx.append(lfx[0])#[11,7]#lfx
        ...
        models.append(model.initmodel(cfg.fy[-1],cfg.fx[-1],cfg.lev[-1],cfg.useRL,cfg.deform,numbow,CRF=cfg.CRF,small2=cfg.small2))
    #check model
...
#        newtrnegcl=[]
        ...
        totPosEx=0
        for i in range(len(final_trPosImages)):
                totPosEx += len(final_trPosImages[i]["bbox"])  
        ...
        for it in range(cfg.posit):
            ...
            if cfg.bestovr and it==0:#force to take best overlapping
                ...
                cfg.mpos=0#0.5
                cfg.posovr=auxposovr
                cfg.ranktr=temprank
                trPosImages=final_trPosImages
                cfg.initr=auxinitr
