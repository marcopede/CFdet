...
def detectWrap(a):
    ...
    res=pyrHOG2.detect(f,m,gtbbox,hallucinate=cfg.hallucinate,initr=cfg.initr,ratio=cfg.ratio,deform=cfg.deform,posovr=cfg.posovr,bottomup=cfg.bottomup,usemrf=cfg.usemrf,numneg=cfg.numneg,thr=cfg.thr,inclusion=cfg.inclusion,small=False,show=cfg.show,usefather=cfg.usefather,emptybb=True,useprior=False)
    if cfg.show:
        pylab.show()
#        raw_input()
    ...
    for it in range(cfg.numit):
        ...
        #res=it.next()
            if it==0:
                ...
                nb=len(res[1])
                for idel,el in enumerate(res[1]):
                    print "BBox:",numbb
                    if res[1][idel]!=[]: #if got a detection for the bbox
                        if trpos[numbb+idel]==[]: 
                            print "Added example previuosly empty!"
                            trpos[numbb+idel]=res[3][idel]
                            trpos[numbb+nb+idel]=res[3][nb+idel]
                        if numpy.sum(trpos[numbb+idel]*w)-r<res[1][idel]["scr"]:
                            print "New score (%f) higher than previous (%f)!"%(res[1][idel]["scr"],numpy.sum(trpos[numbb+idel]*w)-r)
                            trpos[numbb+idel]=res[3][idel]
                            trpos[numbb+nb+idel]=res[3][nb+idel]
                        else:
                            print "Keep old example because better score!"
